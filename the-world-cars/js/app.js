/* ==========================================================================
   THE WORLD CARS - Main UI Engine (v2.1)
   Includes: Dynamic Site Config Binding, Strict Date Validation & Clean UI Buttons
   ========================================================================== */
...
// Secret Keyboard Shortcut (Ctrl + Shift + A) to open Admin Modal
...
// Dynamic Site Configuration Binder (Binds phone, emergency, email, whatsapp, address across UI)
window.applySiteConfigToUI = function() {
  ...
  document.querySelectorAll('.ui-address').forEach(el => el.textContent = config.address);
};
...
// Open Car Modal & Dynamic Date Validator
window.openCarModal = function(carId) {
  ...
  // Dynamic Date Validation & Calculation
  ...
  endDateInput.addEventListener('change', () => {
    if (startDateInput.value && endDateInput.value < startDateInput.value) {
      endDateInput.value = startDateInput.value;
      showToast("La data di fine non può essere precedente alla data di inizio!", "emergency");
    }
    calculateEstimate();
  });
