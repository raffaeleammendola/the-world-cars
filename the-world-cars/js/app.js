...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Gestione Swipe Indietro (Mobile Modal chiusura)
...
window.applySiteConfigToUI = function() {
  ...
  if (filtersContainer) {
    let html = `<button class="filter-btn active" onclick="filterCategory('Tutte', this)">Tutte le Vetture</button>`;
    cats.forEach(c => {
...
window.openCarModal = function(carId) {
  ...
  history.pushState({ modal: 'car-modal' }, '');
};
  ...
  document.querySelectorAll('.modal-close').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (modal && modal.classList.contains('active')) {
        history.back(); // Questo triggera popstate che chiude il modale
      }
    });
  });
...
window.goToCarousel = function(carId, index) {
  ...
  if (track) track.style.transform = `	ranslateX(-%)`;
