...
function initDropzoneUploaders() {
  document.querySelectorAll('.dropzone').forEach(dropzone => {
    ...
    const targetUrlInput = dropzone.parentElement.querySelector('input[type="text"]');
...
function renderAdminRequestsTable() {
  ...
  container.innerHTML = filtered.map(r => `
    <tr>
      ...
      <td>${r.serviceName || '-'}</td>
      <td>${r.startDate ? `${formatDateIT(r.startDate)} ${r.endDate ? '➜ ' + formatDateIT(r.endDate) : ''}` : (r.createdAt || '-')}</td>
      <td>
...
window.changeReqStatus = function(id, newStatus) {
  ...
  // Quando lo stato cambia a 'Confermata', invia AUTOMATICAMENTE WhatsApp + Email al cliente
  if (newStatus === 'Confermata' && req) {
    const cleanPhone = (req.phone || '').replace(/[^0-9]/g, '');
    const clientMsg = `✅ Gentile ${req.clientName}, la tua richiesta per ${req.serviceName || req.type} è stata CONFERMATA! 🚗💨\n\nStiamo arrivando / Il tuo appuntamento è stato confermato.\nGrazie per aver scelto The World Cars!`;

    // 1. Apri WhatsApp verso il CLIENTE automaticamente
    if (cleanPhone.length >= 8) {
      const waUrl = `https://wa.me/${cleanPhone}?text=${encodeURIComponent(clientMsg)}`;
      window.open(waUrl, '_blank');
    }

    // 2. Invia Email di conferma al CLIENTE via FormSubmit (diretta, senza server)
    if (req.email && req.email.includes('@')) {
      const formData = new FormData();
      formData.append('_subject', '✅ Prenotazione Confermata - THE WORLD CARS');
      formData.append('_template', 'box');
      formData.append('_captcha', 'false');
      formData.append('Stato', '✅ CONFERMATA');
      formData.append('Gentile', req.clientName);
      formData.append('Servizio', req.serviceName || req.type);
      formData.append('Messaggio', `Siamo lieti di informarti che la tua richiesta per ${req.serviceName || req.type} è stata CONFERMATA dal nostro staff! I nostri operatori ti stanno raggiungendo o ti attendono in sede. Per qualsiasi urgenza contattaci. Cordiali Saluti, THE WORLD CARS`);

      fetch(`https://formsubmit.co/ajax/${req.email}`, {
        method: 'POST',
        headers: { 'Accept': 'application/json' },
        body: formData
      }).then(r => r.json()).then(result => {
        if (result.success) {
          console.log(`✅ Email conferma inviata al cliente ${req.email}`);
        } else {
          console.warn("FormSubmit client email:", result);
        }
      }).catch(e => console.error("Email cliente error:", e));
    }

    alert(`✅ Richiesta di ${req.clientName} CONFERMATA!\n\n• WhatsApp al cliente: ${cleanPhone.length >= 8 ? 'INVIATO' : 'Numero non valido'}\n• Email al cliente: ${req.email ? 'INVIATA a ' + req.email : 'Nessuna email fornita'}`);
  }
};
...
window.updateCarCategorySelects = function(cats) {
...
window.openEditCarModal = function(carId) {
  ...
  form.elements['luggage'].value = car.luggage || '2 Valigie';
  ...
  form.elements['image2'].value = car.image2 || '';
  form.elements['image3'].value = car.image3 || '';
  form.elements['ac'].value = car.ac === true ? 'true' : 'false';
  form.elements['cv'].value = car.cv || '';
  form.elements['description'].value = car.description || '';

  // Update image preview for image 1
  const dropzones = form.querySelectorAll('.dropzone');
  if (dropzones[0]) {
    const preview1 = dropzones[0].parentElement.querySelector('.dropzone-preview');
    if (preview1 && car.image) {
      preview1.src = car.image;
      preview1.style.display = 'block';
    }
  }
  // Update image preview for image 2
  if (dropzones[1]) {
    const preview2 = dropzones[1].parentElement.querySelector('.dropzone-preview');
    if (preview2 && car.image2) {
      preview2.src = car.image2;
      preview2.style.display = 'block';
    } else if(preview2) {
      preview2.style.display = 'none';
    }
  }
  // Update image preview for image 3
  if (dropzones[2]) {
    const preview3 = dropzones[2].parentElement.querySelector('.dropzone-preview');
    if (preview3 && car.image3) {
      preview3.src = car.image3;
...
window.handleSettingsSubmit = async function(e) {
  ...
  const updatedConfig = {
    ...
    supabaseUrl: formData.get('supabaseUrl') ? formData.get('supabaseUrl').trim() : '',
    supabaseKey: formData.get('supabaseKey') ? formData.get('supabaseKey').trim() : '',
    carCategories: formData.get('carCategories').trim()
  };
