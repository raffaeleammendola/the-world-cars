...
class AppStore {
  ...
  async initCloudSync() {
    ...
    try {
      ...
      // ── CONFIGURAZIONE SITO (WhatsApp, Email, Telefono, ecc.) ──
      const dbConf = await this._get('configurazione', '?id=eq.1');
      if (dbConf && dbConf.length > 0) {
        const c = dbConf[0];
        this.siteConfig = {
          ...this.siteConfig,
          companyName: c.nome_azienda || this.siteConfig.companyName,
          address: c.indirizzo || this.siteConfig.address,
          phone: c.telefono || this.siteConfig.phone,
          emergencyPhone: c.telefono_emergenza || this.siteConfig.emergencyPhone,
          whatsappNumber: c.whatsapp || this.siteConfig.whatsappNumber,
          email: c.email || this.siteConfig.email,
          logoIcon: c.logo_icona || this.siteConfig.logoIcon,
          logoImageUrl: c.logo_immagine || this.siteConfig.logoImageUrl,
          telegramBotToken: c.telegram_bot_token || this.siteConfig.telegramBotToken,
          telegramChatId: c.telegram_chat_id || this.siteConfig.telegramChatId,
          adminPasswordHash: c.password_hash || this.siteConfig.adminPasswordHash,
          carCategories: c.categorie_auto || this.siteConfig.carCategories
        };
        this.saveToStorage('twc_config', this.siteConfig);
        console.log("✅ Configurazione sito caricata dal Cloud (WhatsApp:", this.siteConfig.whatsappNumber + ")");
      }
  ...
  async sendEmailNotification(payload) {
    ...
    console.log(`📧 Invio email a ${targetEmail} via FormSubmit...`);

    try {
      // Invio DIRETTO dal browser via FormSubmit (non passa dal server Vercel)
      const formData = new FormData();
      formData.append('_subject', `🏎️ THE WORLD CARS: ${payload.type || 'Richiesta'} - ${payload.clientName || 'Cliente'}`);
      formData.append('_template', 'table');
      formData.append('_captcha', 'false');
      formData.append('Tipo_Richiesta', payload.type || 'Generale');
      formData.append('Nome_Cliente', payload.clientName || 'N/D');
      formData.append('Telefono', payload.phone || 'N/D');
      formData.append('Email_Cliente', payload.email || payload.clientEmail || 'Non fornita');
      formData.append('Servizio', payload.serviceName || 'N/D');
      if (payload.startDate) formData.append('Data', `${payload.startDate}${payload.endDate ? ' - ' + payload.endDate : ''}`);
      if (payload.totalPrice) formData.append('Prezzo_Stimato', `€${payload.totalPrice}`);
      if (payload.location) formData.append('Posizione_GPS', payload.location);
      if (payload.notes) formData.append('Messaggio', payload.notes);
      if (payload.isConfirmation) formData.append('Stato', '✅ CONFERMATA');
      ...
      if (result.success) {
        console.log("✅ Email inviata con successo via FormSubmit!");
      } else {
        console.warn("FormSubmit response:", result);
      }
    } catch (e) {
      console.error("Errore invio email FormSubmit:", e);
    }
  }
  ...
  addCar(carData) {
    ...
    this._post('auto', {
      ...
      immagine2: carData.image2 || null, immagine3: carData.image3 || null,
      aria_condizionata: carData.ac || false, cavalli: carData.cv || null,
  ...
  updateCar(id, data) {
    ...
    this._patch('auto', `id_locale=eq.${id}`, {
      ...
      immagine2: data.image2, immagine3: data.image3,
      aria_condizionata: data.ac, cavalli: data.cv,
  ...
  async updateConfig(newConfig) {
    ...
    // Salva la configurazione su Supabase (così è uguale su tutti i dispositivi)
    this._patch('configurazione', 'id=eq.1', {
      nome_azienda: this.siteConfig.companyName,
      indirizzo: this.siteConfig.address,
      telefono: this.siteConfig.phone,
      telefono_emergenza: this.siteConfig.emergencyPhone,
      whatsapp: this.siteConfig.whatsappNumber,
      email: this.siteConfig.email,
      logo_icona: this.siteConfig.logoIcon,
      logo_immagine: this.siteConfig.logoImageUrl,
      telegram_bot_token: this.siteConfig.telegramBotToken,
      telegram_chat_id: this.siteConfig.telegramChatId,
      password_hash: this.siteConfig.adminPasswordHash,
      categorie_auto: this.siteConfig.carCategories
    });
