/* ==========================================================================
   THE WORLD CARS - Central Production Store & Relational Cloud Sync Engine (v2.0)
   Supports: Relational Supabase Schema v2.0 + Direct & Serverless Telegram Dispatcher
   ========================================================================== */
...
class AppStore {
  ...
  async verifyAdminPassword(plainTextPassword) {
    const hashedInput = await this.hashPassword(plainTextPassword);
  ...
  // --- Telegram Notification Dispatcher (Serverless + Local Fallback) ---
  async sendTelegramNotification(payload) {
    ...
    // 1. Try Vercel Serverless Endpoint (/api/notify)
    try {
      ...
      console.log("Serverless endpoint unavailable on local server, using direct Telegram API fallback.");
    }

    // 2. Direct Telegram API Fallback (for Localhost development & client testing)
    ...
    if (token && chatId) {
      try {
        ...
        if (directData.ok) {
          console.log("✅ Direct Telegram notification dispatched successfully!");
          return { success: true };
        } else {
          console.error("❌ Telegram Direct API Error:", directData);
        }
      } catch (err) {
  ...
  // --- Relational Supabase Sync (v2.0 Schema) ---
  async syncSupabaseRelational(table, payload) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log(`✅ Supabase v2.0 sync success for ${table}:`, data);
        return Array.isArray(data) ? data[0] : data;
      }
    } catch (e) {
      console.warn("Supabase Sync v2.0 warning:", e);
    }
    return null;
  }

  // --- Relational Request Processor ---
  async createRequest(reqData) {
    ...
    // Step C: Insert into specific service table
    ...
    // 3. Dispatch Telegram Alert (with Automatic Fallback)
  ...
  async updateConfig(newConfig) {
    ...
    if (newConfig.newPassword && newConfig.newPassword.trim() !== '') {
      newConfig.adminPasswordHash = await this.hashPassword(newConfig.newPassword.trim());
      delete newConfig.newPassword;
    }
