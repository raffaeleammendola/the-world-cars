  ...
  async initCloudSync() {
    ...
    try {
      ...
      if (resCars.ok) {
        ...
        // Sovrascrivi SEMPRE con i dati del cloud (anche se vuoti), per evitare che auto fantasma rimangano in locale
  ...
  async syncSupabaseRelational(table, payload) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("Supabase POST error:", errText);
        alert(`❌ Errore durante il salvataggio in Cloud (${table}). Assicurati di aver incollato il codice SQL in Supabase! Dettagli: ${errText.substring(0,100)}`);
      }
    } catch (e) {
      console.warn("Supabase Sync warning:", e);
      alert(`❌ Impossibile connettersi al Cloud Supabase. Verifica la tua connessione.`);
    }
    return null;
  }
